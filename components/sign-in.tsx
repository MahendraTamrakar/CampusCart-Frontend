  ...
  const handleSignIn = async (e: FormEvent) => {
    ...
    if (validateForm(signInData, 'signIn')) {
      try {
        ...
        setErrors(prev => ({
          ...prev,
          signIn: { ...prev.signIn, email: error.response?.data?.message || 'Invalid email or password' }
        }));
      }
    }
  };
      ...
      <div ref={containerRef} className={`container bg-white rounded-[30px] shadow-[0_5px_15px_rgba(0,0,0,0.35)] relative overflow-hidden w-[768px] max-w-full min-h-[480px] ${isActive ? 'active' : ''}`}>
        <div className="form-container sign-up absolute top-0 h-full transition-all duration-[0.6s] ease-in-out left-0 w-1/2 opacity-0 z-[1]">
          <form onSubmit={handleSignUp} className="bg-white flex items-center justify-center flex-col px-10 h-full">
            ...
            <input 
              ...
              onChange={handleSignUpChange}
            />
            <button type="submit" className="bg-[#512da8] text-white text-xs py-2.5 px-[45px] border border-transparent rounded-lg font-semibold tracking-[0.5px] uppercase mt-2.5 cursor-pointer transition-transform hover:scale-105">Sign Up</button>
          </form>
        </div>
        <div className="form-container sign-in absolute top-0 h-full transition-all duration-[0.6s] ease-in-out left-0 w-1/2 z-[2]">
          <form onSubmit={handleSignIn} className="bg-white flex items-center justify-center flex-col px-10 h-full">
            ...
            <button type="submit" className="bg-[#512da8] text-white text-xs py-2.5 px-[45px] border border-transparent rounded-lg font-semibold tracking-[0.5px] uppercase mt-2.5 cursor-pointer transition-transform hover:scale-105">Sign In</button>
          </form>
        </div>
        <div className="toggle-container absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-all duration-[0.6s] ease-in-out rounded-[150px_0_0_100px] z-[1000]">
